					...
					try {
							...
							if (devs && devs.length) {
									...
									yield Promise.all(devs.map((() => {
											var _ref3 = _asyncToGenerator(function* (dev) {

													const repos = yield gh.getRepos(dev);

													if (repos && repos.length) {

															const stars = repos.reduce(function (counter, _ref4) {
																	let lang = _ref4.language,
...
/***/ function(module, exports, __webpack_require__) {
	...
	class Github {
		...
		getData(url) {
			...
			return (0, _nodeFetch2.default)(url, { headers: headers }).then(res => {
